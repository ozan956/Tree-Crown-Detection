...
\midrule
Density-map counter$^\dagger$ & 0.888 & 0.834 & 0.860 & n/a & n/a \\
\midrule
...
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize $^\dagger$Density-map counter (CSRNet-style regression) trained on
the same data as a modern baseline; it is a strong \emph{counter} (test count
MAE 4.63 trees/image) but a weaker \emph{detector}, its best detection $F_1$
(0.860) falling below both the ensemble and the integrated method. Peak-decode
thresholds were tuned for best $F_1$.}
\end{table}
...
reliable detector box unconditionally. Trained and evaluated on image-disjoint
halves of the test set, the gate has a tunable acceptance threshold that trades
precision against the retention of the beyond-ceiling recoveries. Because those
recoveries are the paper's central finding, we adopt a \emph{recovery-preserving}
operating point that keeps 94\% of them while still improving precision to 0.893
(recall 0.957, $F_1 = 0.924$), which already exceeds the WBF ensemble's 0.921 at
a recall the ensemble cannot reach. A more aggressive threshold attains
$F_1 = 0.934$ (precision 0.962, recall 0.908) but retains only 38\% of the
beyond-ceiling recoveries; we therefore do not adopt it as primary. At the
recovery-preserving point the integrated detector \emph{dominates} the ensemble
on the precision--recall frontier while preserving the complementary detections
